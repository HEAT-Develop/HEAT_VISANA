...
class MeshVisualizer:
    ...
    def run_analysis_without_plotting(self, picked_points):
        ...
        boundary_pts_2 = results["Boundary Points"]
    ...
    #this is working still not finished
    ...
    def compute_rim_path(self, picked_points):
        ...
        mesh     = self.mesh_loader.mesh
        results  = {}
        results["Seed Points"] = picked_points
        ...
        #points to 2d
        pts2d_full = pca.transform(boundary_points)
        x = pts2d_full[:, 0]
        y = pts2d_full[:, 1]
        ...
        area_2d = 0.5 *np.sum(cross) #not giving good value
        denom6 = 6.0 * (area_2d if area_2d != 0 else 1e-12)
        cx_2d = np.sum((x + x_next) * cross) / denom6
        cy_2d = np.sum((y + y_next) * cross) / denom6
        centroid_2d = np.array([[cx_2d, cy_2d]])
        rim_center_3d = pca.inverse_transform(centroid_2d)[0]
        
        seg_len = np.hypot(x_next - x, y_next - y)
        P_2d = seg_len.sum()

        rim_area_m2 = abs(area_2d)*(1000.0**2)

        u2_to_m = 1000.0
        perim_m  = P_2d * u2_to_m 
        circularity = 4*np.pi*rim_area_m2 / (perim_m**2)
        D_eq = 2.0 * np.sqrt(rim_area_m2 / np.pi)

        vertices_count = int(len(boundary_points))
        #print(f"Boundary Faces: {boundary_faces}")

        results["rim_center"]       = rim_center_3d
        results["rim_area"]         = rim_area_m2
        results["vertices_count"]   = vertices_count
        results["rim_polygon_ids"]  = boundary_faces
