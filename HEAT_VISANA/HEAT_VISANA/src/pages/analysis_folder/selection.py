...
from PyQt6.QtWidgets import QLabel,QDialog, QApplication,QMessageBox,QLineEdit, QPushButton, QVBoxLayout, QWidget, QHBoxLayout,QDateTimeEdit, QComboBox, QSizePolicy
...
from src.pages.analysis_folder.model_add_asteroid_info import AddInfoRegionDialog
import os
...
class Selections(QWidget):
    def __init__(self, parent=None):
        ...
        self.current_mesh_path  = str #Path of File
    ...
    def initUI(self):
        ...
        btn_update_db = QPushButton("Append info to DB")
        ...
        btn_update_db.clicked.connect(self.open_modal)
    ...
    def search_coordinates(self):
        ...
        self.base_actor = self.view.plotter.add_mesh(
            self.mesh,
            scalars=self.active_scalars if hasattr(self, "active_scalars") else None,
            cmap=getattr(self, "active_cmap", None),
            reset_camera=False,
            pickable=True,
            name="base_mesh",
        )
        for idx, (lat, lon) in enumerate(zip(self.mesh["latitude"], self.mesh["longitude"])):
            ...
            if lat_str.startswith(sel_lat) and lon_str.startswith(sel_lon):
                ...
                sphere = pv.Sphere(radius=0.003, center=center)
    ...
    #-----------Open-Modal------------#
    def open_modal(self):
        created_at_qdt = self.date_time_edit.dateTime() if hasattr(self, "date_time_edit") else None
        
        # polygons = getattr(getattr(self, "rigion_type", None), "polygons", None)

        # pass a callable, not the result! (if you need a specific name, bind it)
        download_cb = lambda path: self.download_vtk(path)  
        dlg = AddInfoRegionDialog(self, mesh=getattr(self, "mesh", None),
                                mesh_path=getattr(self, "current_mesh_path", ""),
                                created_at_qdt=created_at_qdt,
                                polygons=self.new_selected_cells,
                                download_vtk_cb=download_cb, )
        
        if dlg.exec() == QDialog.DialogCode.Accepted:
            asteroid_name = dlg.asteroid_name.text()
            dlg.mesh = self.mesh    
            dlg.mesh_path = self.current_mesh_path
    ...
    def callback_cell_pick(self, unstructuredg_rid):
        ...
        self.new_selected_cells = new_selected_cells
        curvature = self.mesh.curvature(curv_type='mean')
        clamped_curvature = np.clip(curvature, -100, 200)
        self.mesh.point_data['Curvature'] = clamped_curvature
    ...
    def update_region_data(self, surface_mesh, unstructuredg_rid):
        ...
        print("BRIGNE ------------>", self.all_temperatures)
    ...
    def update(self):
        self.plot()
    #------------END--------------#
    #---------DOWNLAOD-VTK---------##REVISE THIS PART
    def _normalize_output_path(self, path_str: str, base_dir: str | None = None) -> str:
        ...
        if path_str.startswith("file://"):
            ...
            if not os.path.isabs(path):
                # choose your project root as base if you like
    ...
    def download_vtk(self, out_path_like: str):
        path = self._normalize_output_path(out_path_like, base_dir=os.getcwd())
        # Write file (replace with your own writer if needed)
        # Example uses your existing object:
        self.rigion_type.write_vtk_polydata(path)

        print("VTK written to:", path)
        # Optional: verify
        if not os.path.exists(path):
            print("WARNING: writer reported success but file not found at:", path)
    #-------------END--------------#
    ...
    def upload_model (self):
        ...
        self.current_mesh_path = filename
        #self.current_mesh_uri  = Path(filename).resolve().as_uri()
