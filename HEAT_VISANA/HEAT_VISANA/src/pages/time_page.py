    ...
    def load_meshes(self, view_index, index):
       ...
       #self.vtk_data_cache = {}  # Reset cache
        for i, filepath in enumerate(self.files):
            print(f"[INFO] Processing file {i + 1} of {len(self.files)}: {filepath}")  # Log the number and file
            if filepath not in view["vtk_data_cache"]:
                ...
                print(f"[INFO] File {i + 1} of {len(self.files)} is already cached.")
